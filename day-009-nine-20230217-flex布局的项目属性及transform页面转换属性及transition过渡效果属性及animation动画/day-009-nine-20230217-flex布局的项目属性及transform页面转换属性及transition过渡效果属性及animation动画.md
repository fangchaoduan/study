...
- 容器与项目的概念
  - 设置了`display:flex;`的就是容器。容器内部的子元素就是容器的项目。
- 容器与项目的多层
  - flex布局并不会继承，所以设置在容器上的属性并不会继承给子元素。
  - 一个容器内的元素也可以是它自身子元素的容器，只要在项目上设置`display:flex;`就可以了。
...
- scale 缩放
  - `transform:scale(x,y)`
    - x代表的是缩放宽度的x倍
    - y代表的是缩放高度的y倍
  - `transform:scale(n)`  同时缩放宽度和高度的n倍，是`transform:scale(n,n)`的简写
...
1. `:hover`等伪类
2. 用JavaScript给元素增加删除class类名导致样式变动。
3. 用JavaScript修改css样式。
4. 子元素新增或删除导致的宽高变化等
